...
import {
  ...
  Table, ActionIcon, Tooltip, Skeleton, Alert, TextInput,
} from '@mantine/core';
import { IconPlus, IconEdit, IconBan, IconEye, IconSearch, IconFileText } from '@tabler/icons-react';
...
export default function RemitosPDV() {
  ...
  const rows = useMemo(() => filtrados.map((r) => (
    <Table.Tr key={r.id}>
      <Table.Td><Text fw={600}>#{r.numero}</Text></Table.Td>
        ...
        <Group gap="xs">
          ...
          {r.estado !== 'anulado' && (
            <Tooltip label="Editar">
              <ActionIcon variant="light" color="orange" onClick={() => navigate(`/pdv/remitos/${r.id}/editar`)}>
                <IconEdit size={16} />
              </ActionIcon>
            </Tooltip>
          )}
  ...
  return (
    <Stack gap="lg">
      <Group justify="space-between" align="flex-end">
        <div>
          <Title order={2} fw={600} style={{ letterSpacing: '-0.02em' }}>Mis Remitos</Title>
          <Text c="dimmed" size="sm" mt={2}>Remitos generados desde este punto de venta</Text>
        </div>
        <Button leftSection={<IconPlus size={14} />} color="green" size="sm" onClick={() => navigate('/pdv/remitos/nuevo')}>
          Nuevo Remito
        </Button>
      </Group>
      ...
      <Paper style={{ border: '1px solid #e9ecef', overflow: 'hidden', background: '#fff' }}>
        {cargando ? (
          ...
          <Alert icon={<IconFileText size={16} />} m="md" color="blue" variant="light">
            {remitos.length === 0
              ? 'Todavía no generaste ningún remito. ¡Empezá creando uno!'
